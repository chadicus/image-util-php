...
final class Image
{
    ...
    public static function resizeMulti(\Imagick $source, array $boxSizes, array $options = [])
    {
        ...
        //the photography field, while being reasonably fast. Upsizing is more subjective but you can't get much better
        //than bicubic which is what is used here.
        ...
        foreach ($boxSizes as $boxSizeKey => $boxSize) {
            ...
            //only relevant once written. Imagick::stripImage() doesnt even have an effect until written also the user
            //can just call that function with the resultant $canvas
     ...
     * @param array $options options
     *     string format (default jpeg) format. Any supported from
     ...
     *     bool stripHeaders (default true) whether to strip headers (exif, etc). Is only reflected in $destPath, not
     *                       returned clone
     *
     * @return void
     *
